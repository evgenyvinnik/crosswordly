...
test.describe('GameCompletionModal', () => {
  ...
  test('should download crossword as PNG when Download button is clicked', async ({ page }) => {
    ...
    expect(download.suggestedFilename()).toMatch(/crossword.*\.png$/i);

    // Save and verify the file
    const tempPath = path.join(__dirname, '..', '..', 'test-results', download.suggestedFilename());
    ...
    // Verify it's a PNG file (check magic number)
    ...
    // Verify modal is still open (download shouldn't close it)
